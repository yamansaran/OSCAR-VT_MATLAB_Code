...
%% ====================== LOAD VEHICLE CONFIG FROM RCS_CONFIG.MAT =========
...
ast.n_thr     = cfg.asterix.n_thrusters;
...
fprintf('  %s (chaser): %.1f kg, %d thr (%s), [%.2f x %.2f x %.2f] m\n', ...
        ast.name, ast.mass, ast.n_thr, ast.ctrl_mode, ast.Lx, ast.Ly, ast.Lz);
fprintf('  %s (target): %.1f kg, %d thr (%s), [%.2f x %.2f x %.2f] m\n', ...
        obx.name, obx.mass, obx.n_thr, obx.ctrl_mode, obx.Lx, obx.Ly, obx.Lz);
if isfield(cfg, 'timestamp')
    fprintf('  Config generated: %s\n', cfg.timestamp);
end

%% ====================== DOCKING MECHANISM CONFIG ========================
...
mech.k = 800;           % [N/m] axial stiffness of mechanism + structure
...
% STRUCTURAL LIMITS
struct_lim.F_max   = 300;    % [N] max allowable axial contact force
...
capture_geom.cone_half_angle = 25;   % [deg] drogue cone half-angle
...
N_mc = 5000;             % Monte Carlo samples per speed point
...
fprintf('\n=== Docking Contact Dynamics: %s (chaser) -> %s (target) ===\n\n', ...
        ast.name, obx.name);
fprintf('%s: %.1f kg   %s: %.1f kg   Reduced mass: %.2f kg\n', ...
        ast.name, m_chaser, obx.name, m_target, m_red);
fprintf('Mechanism: %s\n', mech.type);
...
v_sweep_lo = max(v_min_energy * 0.3, 0.001);
v_sweep_hi = min(v_max_energy * 2.0, 1.0);     % cap at 1 m/s sanity limit
v_sweep = linspace(v_sweep_lo, v_sweep_hi, 300);
...
sgtitle(sprintf('Contact Analysis: %s (%.0f kg) -> %s (%.0f kg), %s', ...
        ast.name, m_chaser, obx.name, m_target, mech.type));
...
v_mc_lo = 0.050;  % 50 mm/s
v_mc_hi = 0.250;  % 250 mm/s
...
figure('Name','Monte Carlo Capture Probability','Position',[100 100 900 600]);
hold on; grid on;

fill_between_x = [v_mc_sweep*1e3 fliplr(v_mc_sweep*1e3)];
fill(fill_between_x, [P_success zeros(size(P_success))], ...
     [0.3 0.8 0.3], 'FaceAlpha', 0.2, 'EdgeColor', 'none', 'HandleVisibility','off');

plot(v_mc_sweep*1e3, P_probe_in, '--', 'Color', [0.5 0.5 0.8], ...
     'LineWidth', 1.5, 'DisplayName', 'P(probe enters drogue)');
plot(v_mc_sweep*1e3, P_capture, '--', 'Color', [0.2 0.6 0.9], ...
     'LineWidth', 1.5, 'DisplayName', 'P(latch capture)');
plot(v_mc_sweep*1e3, P_force_ok, '--', 'Color', [0.9 0.5 0.2], ...
     'LineWidth', 1.5, 'DisplayName', 'P(force within limits)');
plot(v_mc_sweep*1e3, P_success, 'k-', 'LineWidth', 2.5, ...
     'DisplayName', 'P(success) = all criteria');

yline(0.95, 'r--', 'LineWidth', 1.5 , 'HandleVisibility','off');
xline(v_optimal*1e3, 'g-', 'LineWidth', 2 , 'HandleVisibility','off');

if ~isnan(v_95_low)
    fill([v_95_low v_95_high v_95_high v_95_low]*1e3, ...
         [0 0 1 1], [0.3 0.9 0.3], 'FaceAlpha', 0.1, 'EdgeColor', 'g', ...
         'LineWidth', 1.5, 'HandleVisibility', 'off');
end

xlabel('Nominal Approach Speed [mm/s]');
ylabel('Probability');
title(sprintf('Capture Probability vs Approach Speed (N=%d per point)', N_mc));
...
fprintf('================================================================\n');
fprintf('  Chaser: %s (%.1f kg, %d thr, %s)\n', ...
        ast.name, m_chaser, ast.n_thr, ast.ctrl_mode);
fprintf('  Target: %s (%.1f kg, %d thr, %s)\n', ...
        obx.name, m_target, obx.n_thr, obx.ctrl_mode);
fprintf('  Reduced mass:   %.1f kg\n', m_red);
fprintf('  RCS config:     %s\n', rcs_mat_file);
