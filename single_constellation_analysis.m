...
% =========================================================================
J2_SELECTED_ALTITUDES_KM_PER_PC = [200];  % <-- EDIT THIS
...
% =========================================================================
OPERATION_DV_PER_SAT        = [110,110,110];  % <-- EDIT THIS (length = nSatellites)
OPERATION_TIME_DAYS_PER_SAT = [5,5,5];  % <-- EDIT THIS (length = nSatellites)
...
if ~isempty(J2_SELECTED_ALTITUDES_KM_PER_PC)
    if length(J2_SELECTED_ALTITUDES_KM_PER_PC) ~= nPlaneChanges
        ...
        altTimeline  = [];  % [time_s, altitude_km] for altitude-vs-time plot (init on first plane)
        ...
        for planeVisit = 1:nPlanes
            ...
            chaserAlt_km = (a_plane - R_EARTH) / 1e3;  % current plane altitude
            ...
            if planeVisit == 1
                ...
                altTimeline = [0, chaserAlt_km];  % initialize altitude timeline
            end
            ...
            for si = 1:nSatsInPlane
                ...
                if satCounter == 1 && (INSERTION_DV > 0 || INSERTION_TIME_DAYS > 0)
                    ...
                    if INSERTION_DV > 0 || INSERTION_TIME_DAYS > 0
                        ...
                        altTimeline  = [altTimeline;  selTime, chaserAlt_km];
                    end
                else
            ...
            if planeVisit < nPlanes
                ...
                % Altitude of next plane (destination after this maneuver)
                a_nextPlane = mean([nextPlane.satellites.a]);
                nextPlaneAlt_km = (a_nextPlane - R_EARTH) / 1e3;
                % J2 maneuver altitude (the intermediate parking orbit)
                j2ParkAlt_km = chaserAlt_km + chosenAlt;
                ...
                if j2opt.totalTime >= Inf
                    ...
                    % Edelbaum fallback: stays at roughly nominal altitude
                    % throughout the active burn, then arrives at next plane
                    altTimeline = [altTimeline; selTime, chaserAlt_km];
                    altTimeline = [altTimeline; selTime + pcTime_s, nextPlaneAlt_km];
                else
                    pcDV = j2opt.dv_total;
                    ...
                    % =====================================================
                    % ALTITUDE PROFILE THROUGH J2 MANEUVER
                    % Leg 1: spiral from nominal alt to J2 parking alt
                    % Coast:  hold at J2 parking alt while RAAN drifts
                    % Leg 2: spiral back from parking alt to next plane alt
                    % =====================================================
                    t_half_spiral = t_spiral_s / 2;
                    % before spiral: still at current plane alt
                    altTimeline = [altTimeline; selTime, chaserAlt_km];
                    % after spiral up (or down): at J2 parking altitude
                    altTimeline = [altTimeline; selTime + t_half_spiral, j2ParkAlt_km];
                    % end of coast: still at J2 parking altitude
                    altTimeline = [altTimeline; selTime + t_half_spiral + t_wait_s, j2ParkAlt_km];
                    % after spiral back: at next plane altitude
                    altTimeline = [altTimeline; selTime + pcTime_s, nextPlaneAlt_km];
                end
                ...
                % Update chaser altitude to next plane after the maneuver
                chaserAlt_km = nextPlaneAlt_km;
                altTimeline  = [altTimeline; selTime, chaserAlt_km];
            end
        ...
        plotAltitudeTimeline(altTimeline, perSat, satArrivalTime, J2_SELECTED_ALTITUDES_KM_PER_PC, nPlaneChanges);
    end
end
...
% are incurred at the client.
    ...
    for k = 1:nS
        ...
        text(satArrivalTime(k)/86400, satArrivalDV(k)/1000 + max(selCumDV/1000)*0.02, ...
            "     "+rawName, 'FontSize', 12, 'Rotation', 0, ...
...
function plotAltitudeTimeline(altTimeline, perSat, satArrivalTime, selectedAlts, nPlaneChanges)
% Plot chaser altitude (or geocentric radius) vs mission time.
%   altTimeline     - Nx2 array [time_s, altitude_km]
%   perSat          - per-satellite struct array (for labels)
%   satArrivalTime  - 1xN vector, mission time (s) at each satellite arrival
%   selectedAlts    - J2 altitude changes per plane change (km)
%   nPlaneChanges   - number of plane changes

    % =====================================================================
    % DISPLAY MODE TOGGLE
    % false = altitude above sea level (km)
    % true  = geocentric radius (km), i.e. altitude + R_Earth
    % =====================================================================
    SHOW_RADIUS = true;  % <-- EDIT THIS (true/false)
    R_EARTH_KM = 6378.137;
    % =====================================================================

    if isempty(altTimeline), return; end
    ...
    figure('Position', [120, 80, 1100, 500]);
    timeDays = altTimeline(:,1) / 86400;
    altKm    = altTimeline(:,2);

    % Apply radius offset if requested
    if SHOW_RADIUS
        altKm = altKm + R_EARTH_KM;
        yLabelStr = 'Geocentric Radius (km)';
        modeStr   = 'Radius';
    else
    ...
    plot(timeDays, altKm, 'b-', 'LineWidth', 2); hold on;

    % Mark satellite arrivals with red dots and name labels
    nS = length(perSat);
    if ~isempty(satArrivalTime)
        for k = 1:nS
            arrDay = satArrivalTime(k) / 86400;
            % Find altitude at arrival by previous-value interpolation
            % Find altitude at arrival by scanning backwards for the
            % last entry at or before the arrival time (avoids the
            % interp1 unique-sample-points requirement)
            arrAlt = altKm(1);
            for jj = size(altTimeline,1):-1:1
                if altTimeline(jj,1) <= satArrivalTime(k) + 0.5
                    arrAlt = altKm(jj);
                    break;
                end
            end
            scatter(arrDay, arrAlt, 80, 'r', 'filled', 'MarkerEdgeColor', 'k');

            % Satellite name label
            rawName = perSat(k).name;
            if length(rawName) > 15, rawName = [rawName(1:12), '...']; end
            text(arrDay, arrAlt + (max(altKm) - min(altKm)) * 0.03, ...
                rawName, 'FontSize', 12, 'Rotation', 55, ...
                'HorizontalAlignment', 'left', 'VerticalAlignment', 'bottom', ...
                'Color', [0.5, 0, 0]);
        end
    end

    % Dashed horizontal lines at each nominal (flat) altitude for reference
    nominalAlts = [];
    for k = 2:length(altKm)
        if abs(altKm(k) - altKm(k-1)) < 0.5 && ...
           (isempty(nominalAlts) || all(abs(nominalAlts - altKm(k)) > 5))
            nominalAlts(end+1) = altKm(k);
        end
    end
    for na = nominalAlts
        yline(na, 'k:', 'LineWidth', 0.5);
    end

    hold off;
    xlabel('Mission Time (days)', 'FontSize', 12);
    ylabel(yLabelStr, 'FontSize', 12);
    title(sprintf('Chaser %s Over Mission Duration | J2 Altitudes: %s', modeStr, altStr), ...
        'FontSize', 13, 'FontWeight', 'bold');
    legend('Altitude Profile', 'Satellite Arrival', 'Location', 'best');
    grid on;

    % Vertical padding
    altRange = max(altKm) - min(altKm);
    if altRange < 10, altRange = 100; end
    ylim([min(altKm) - altRange * 0.1, max(altKm) + altRange * 0.15]);

    saveas(gcf, 'altitude_timeline.png');
    fprintf('Altitude timeline plot saved: altitude_timeline.png\n');
end